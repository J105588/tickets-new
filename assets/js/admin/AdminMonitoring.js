...
if (!AdminLayout.init('monitoring')) throw new Error('Auth failed');
        ...
        class MonitoringDashboard {
            ...
            initializeElements() {
                this.elements = {
                    ...
                    stopMonitoring: { disabled: false }
                };
            }

            bindEvents() {
                this.elements.monitoringToggle.addEventListener('change', (e) => {
                    if (e.target.checked) {
            ...
            updateStatusIndicator() {
                ...
                const toggle = this.elements.monitoringToggle;
                ...
                for (const [seatId, seatInfo] of Object.entries(seatMap)) {
                    if (seatInfo && typeof seatInfo === 'object') {
                        ...
                        if (statusC === 'checked-in') {
                            ...
                            statusC = '確保';
                            statusE = '';
                        } else if (statusC === 'walkin') {
                            statusC = '当日券';
                            statusE = '済';
                        } else if (statusC === 'blocked' || statusC === 'unavailable') {
                            statusC = '使用不可';
                            statusE = '';
            ...
            analyzeSeatData(seatData) {
                ...
                seatData.forEach((seat, index) => {
                    try {
                        ...
                        const colNum = parseInt(colLabel);

                        // --- 修正箇所：S列の14番〜25番、またはblocked/unavailableの座席は計算から完全に除外する ---
                        if (rowLabel === 'S' && colNum >= 14 && colNum <= 25) {
                            // 計算から除外（カウントしない）
                            return;
                        }

                        if (statusC === '使用不可' || statusC === 'blocked' || statusC === 'unavailable') {
                            // 使用不可の座席も座席総数（分母）からは除外する
                            return;
                        }
                        ...
                        if (statusC === '予約済' || statusC === '確保' || statusC === '当日券' ||
                            statusC === 'checked-in' || statusC === 'to-be-checked-in' || statusC === 'reserved' || statusC === 'walkin') {
                            // 予約済み座席としてカウント
                            reservedSeats++;

                            // チェックイン済みかどうかを判定
                            if ((statusC === '予約済' && statusE === '済') || statusC === 'checked-in' || (statusC === '当日券' && statusE === '済')) {
                                checkedInSeats++;
                            }
                        } else if (statusC === '空' || statusC === '' || statusC === 'empty' || statusC === 'available') {
                            emptySeats++;
                        } else {
                            // C列が認識できない値の場合も一旦空席として扱う
                ...
                const buttonsHtml = classes.map(group => {
                    ...
                    return `
                        <button class="class-button" data-group="${group}" onclick="window.MonitoringDashboard.showClassAnalysis('${group}')">
                            <span class="class-name">${group}</span>
